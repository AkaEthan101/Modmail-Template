...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  commands.addInboxThreadCommand('move', '<category:string$>', async (msg, args, thread) => {
    ...
    if (containsRankings[0][1] === 0) {
      thread.postSystemMessage(":x: I can't find a category with that name!");
      return;
    }
    ...
    try {
      ...
      thread.postSystemMessage(`:x: Failed to move thread: ${e.message}`);
      return;
    }
    ...
    if (config.syncPermissionsOnMove) {
      ...
      try {
        ...
        thread.postSystemMessage(`:white_check_mark: This thread has been moved to \`${targetCategory.name.toUpperCase()}\`, but failed to sync permissions: ${e.message}`);
        return;
      }
    }

    thread.postSystemMessage(`:white_check_mark: This thread has been moved to \`${targetCategory.name.toUpperCase()}\`!`);
  });
};
