...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  async function applyScheduledSuspensions() {
    ...
    for (const thread of threadsToBeSuspended) {
      if (thread.status === THREAD_STATUS.OPEN) {
        ...
        await thread.postSystemMessage(`:white_check_mark: **Thread suspended** as scheduled by ${thread.scheduled_suspend_name}. This thread will act as closed until unsuspended - you can do this by saying \`-unsuspend\`.`);
      }
    }
  }
  ...
  commands.addInboxThreadCommand('suspend cancel', [], async (msg, args, thread) => {
    ...
    if (thread.scheduled_suspend_at) {
      ...
      thread.postSystemMessage(`:white_check_mark: Cancelled scheduled suspension.`);
    } else {
      thread.postSystemMessage(`:x: This thread is not scheduled to be suspended.`);
    }
  });
  ...
  commands.addInboxThreadCommand('suspend', '[delay:delay]', async (msg, args, thread) => {
    if (args.delay) {
      ...
      thread.postSystemMessage(`:white_check_mark: This thread will be suspended in ${utils.humanizeDelay(args.delay)}. Use \`${config.prefix}suspend cancel\` to cancel it.`);

      return;
    }

    await thread.suspend();
    thread.postSystemMessage(`:white_check_mark: **Thread suspended**: this thread will act as closed until unsuspended with \`-unsuspend\`.`);
  });

  commands.addInboxServerCommand('unsuspend', [], async (msg, args, thread) => {
    if (thread) {
      thread.postSystemMessage(`:x: This thread is not suspended!`);
      return;
    }

    thread = await threads.findSuspendedThreadByChannelId(msg.channel.id);
    if (! thread) {
      thread.postSystemMessage(`:x: Not in a thread!`);
      return;
    }

    const otherOpenThread = await threads.findOpenThreadByUserId(thread.user_id);
    if (otherOpenThread) {
      thread.postSystemMessage(`:x: Cannot unsuspend this thread - there is another open thread with this user: <#${otherOpenThread.channel_id}>.`);
      return;
    }

    await thread.unsuspend();
    thread.postSystemMessage(`:white_check_mark: **Thread unsuspended!**`);
  });
};
